...
import { getUploadedImages } from "@/app/_actions/presentation/uploaded-image-actions";
import { ScrollArea } from "@/components/ui/scroll-area";
import { Skeleton } from "@/components/ui/skeleton";

const UPLOADED_IMAGES_PAGE_SIZE = 30;

type UploadedImage = Awaited<ReturnType<typeof getUploadedImages>>[number];

interface UploadedImagesGridProps {
  onImageSelect: (image: UploadedImage) => void;
}

export function UploadedImagesGrid({ onImageSelect }: UploadedImagesGridProps) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useInfiniteQuery({
      ...
      queryFn: ({ pageParam }) =>
        getUploadedImages({
          page: pageParam,
          limit: UPLOADED_IMAGES_PAGE_SIZE,
        }),
      initialPageParam: 1,
      getNextPageParam: (lastPage, allPages) =>
        lastPage.length === UPLOADED_IMAGES_PAGE_SIZE
          ? allPages.length + 1
          : undefined,
    });
  ...
  const uploadedImages = data?.pages.flat() ?? [];
  const showEmptyState = !isLoading && uploadedImages.length === 0;
